...
class VideoDatabase:
    ...
    def add_highlight(
        ...
        """Add a highlight segment for a video."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                INSERT INTO highlights (video_id, start_time, end_time, output_path)
                VALUES (?, ?, ?, ?)
            """,
    ...
    def get_highlights(self, video_id: int) -> List[Tuple[float, float, str]]:
        """Get all highlights for a video."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT start_time, end_time, output_path 
                FROM highlights 
                WHERE video_id = ?
            """,
                (video_id,),
